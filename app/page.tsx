...
async function getNFTs({ pageParam }: { pageParam: number }) {
  const res = await fetch(
    `https://qrc721api.amnst.tech/api/nfts?limit=4&page=${pageParam}`
  );
  if (!res.ok) {
...
export default function Home() {
  ...
  const {
    ...
    getNextPageParam: (lastPage) => {
      const totalPages = lastPage.totalPages; // Assuming totalPages is present in the API response
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      <div className="w-full md:w-10/12 m-auto flex mt-5 mb-5 flex-col md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 md:grid-row-1 md:items-center gap-4">
